d3.request("data/precip-anim/geotiffs/000000.nc.tif").responseType("arraybuffer").get(function(error, request) {
  d3.json("data/map.geojson", function(error, coasts) {
      if (error) throw error;
        if (error) throw error;
//here we load the tif to visualize, as well as the border information

  var w = 1920;
  var h = 1080;

//initialize the gtiff. uses geotiff.js
  ...
  var color = d3.scaleSequential(d3.interpolatePlasma)
      .domain(d3.extent(values)); //color scheme for the contour overlay, easy to change/make variable
  ...
  //definitions for dragging/zoom events
  var drag = d3.drag()
...
//drawing overlay
  map.selectAll("path.data")
 .data(overlay(values).map(invert)/*contours(values).map(invert)*/)
  .enter().append("path")
